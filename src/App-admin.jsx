...
// Normaliza coordenadas que pueden venir mal del sheet (con puntos de miles)
// Ej: "-3.338.303.133.706.030" → "-33.38303133706030"
function normCoord(c) {
  if (!c) return null;
  const s = String(c).trim();
  // Si tiene formato normal con un solo punto y máximo 3 dígitos antes
  const num = parseFloat(s);
  if (Math.abs(num) >= 1 && Math.abs(num) <= 180) return num;
  // Si está malformado con múltiples puntos (miles), quitar puntos excepto el primero
  const partes = s.split(".");
  if (partes.length > 2) {
    // -3.338.303.133.706.030 → tomar parte signo + primer + segundo grupo como entero
    const signo = s.startsWith("-") ? "-" : "";
    const limpio = partes.join("");
    // Insertar punto después de los primeros 2-3 dígitos significativos
    const sinSigno = limpio.replace("-","");
    const fixed = signo + sinSigno.slice(0,2) + "." + sinSigno.slice(2);
                    ...
                    <tr key={i}>
                      ...
                      <td>
                        {(() => {
                          ...
                          if (lat === null || lng === null) return <span style={{color:"#94A3B8",fontSize:12}}>Sin GPS</span>;
                          return (
            ...
            {marcOrden.map((m,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",background:"#F8FAFC",borderRadius:10}}>
                ...
                {(() => {
                  const lat = normCoord(m["Latitud"]);
                  const lng = normCoord(m["Longitud"]);
                  if (lat === null || lng === null) return <span style={{fontSize:11,color:"#94A3B8"}}>Sin GPS</span>;
                  return (
                    <a className="gps-link" href={`https://maps.google.com/?q=${lat},${lng}`} target="_blank" rel="noreferrer">
